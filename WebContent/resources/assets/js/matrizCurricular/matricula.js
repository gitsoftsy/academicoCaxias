...
function getDados() {
  $.ajax({
    ...
    .done(function (data) {
      ...
      $("#disciplina").val(
        `${data.gradeCurricular.disciplina.nome} - ${data.gradeCurricular.disciplina.codDiscip}`
      );
      ...
      $("#anoPeriodo").val(
        `${data.periodoLetivo.ano}/${data.periodoLetivo.periodo}`
      );
      ...
      $.ajax({
        url: url_base + "/alunos/semPrematricula?idTurma=" + turmaId,
        type: "GET",
        async: false,
      })
...
function alteraStatus(element) {
  ...
  $.ajax({
    url:
      url_base +
      `/motivoReprovacaoDocumento/${id}${
        status === "S" ? "/desativar" : "/ativar"
      }`,
    type: "put",
    error: function (e) {
      Swal.close();
...
$("#cola-tabela").on("change", 'input[type="checkbox"]', function () {
  ...
  const checkedCheckboxes = $(
    '#cola-tabela input[type="checkbox"]:checked'
  ).length;
...
function enviarCadastro(base64Anexo) {
  const dataInicio =
    $("#inicio").val() != "" ? `${$("#inicio").val()}T15:30:00` : null;
  const dataFim =
    $("#termino").val() != "" ? `${$("#termino").val()}T15:30:00` : null;
