...
$(document).ready(function() {
	...
	$(".searchButton").click(function() {
		...
		filteredData = dadosOriginais.filter(function(item) {
			...
			// Normalize e remova acentuação da propriedade para comparações mais precisas
			return value
				.toString()
				.toLowerCase()
				.normalize("NFD")
	...
	function sortData(column, order) {
		...
		dadosOrdenados.sort(function(a, b) {
			if (column === "nomeCompleto") {
				var valueA = a.pessoa.nomeCompleto.toLowerCase();
				var valueB = b.pessoa.nomeCompleto.toLowerCase();
				if (order === "asc") {
...
function listarDados(dados) {
	var html = dados
		.map(function(item) {
			var codigoInep =
				item.codigoInep === null ? "Não possui" : item.codigoInep;

			var email =
				item.emailInstitucional === "" ? "Não possui" : item.emailInstitucional;
...
function alteraStatus(element) {
	...
	$.ajax({
		url:
			url_base +
			`/professores/${id}${status === "S" ? "/desativar" : "/ativar"}`,
		type: "put",
		error: function(e) {
			Swal.close();
...
const showProfessorTurma = (ref) => {
	...
	window.location.href =
		"turmas-disciplina-professor?matricula=" +
		matriculaProfessor +
		"&id=" +
		id +
		"&cpf=" +
