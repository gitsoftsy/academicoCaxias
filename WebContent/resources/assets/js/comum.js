...
const path_base =
  ...
  "/" +
  $(location).attr("pathname").split("/")[1] +
  "/resources/menu";
...
$(document).ready(function () {
  ...
  $("<script/>", {
    ...
    src: `${contextPath}/resources/assets/js/util/counterTable.js`,
  }).appendTo("head");
  $("<script/>", {
    type: "text/javascript",
    src: `${contextPath}/resources/assets/js/util/getContainerAluno.js`,
  }).appendTo("head");
  ...
  if (
    ...
    url.includes("cadastroConta") == false
  ) {
    ...
    let transacao =
      ...
      $(location).attr("href").split("/")[
        $(location).attr("href").split("/").length - 1
      ];
    ...
    $.ajax({
      ...
      error: function (e) {
        if (
          e.responseText ==
          "Nenhum acesso encontrado para o usuário ou transação informado."
        ) {
      ...
      if (data[0].altera == "N") {
        ...
        $(".edit-val").wrap(
          '<div class="box-edit-val" title="Seu usuário não tem autorização"></div>'
        );

        $(".btn-new-alter").css("cursor", "pointer").attr("disabled", true);
        $(".btn-new-alter").wrap(
          '<div class="box-edit-val px-3 py-1 ms-auto" title="Seu usuário não tem autorização"></div>'
        );

        $(".checkbox-toggle").css("cursor", "pointer").attr("disabled", true);
        $(".checkbox-toggle").wrap(
          '<div class="box-edit-val" onclick="notAccess()" title="Seu usuário não tem autorização"></div>'
        );

        $(".statusAprovacao").css("cursor", "pointer").attr("disabled", true);
        $(".statusAprovacao").wrap(
          '<div class="box-edit-val" title="Seu usuário não tem autorização"></div>'
        );
        ...
        $(".edit-table").wrap(
          '<div class="box-edit-val" style="width:50%; display: flex; align-items:center; justify-content:center" title="Seu usuário não tem autorização"></div>'
        );
  ...
  if (
    window.location.pathname.includes("escola") &&
    !window.location.toString().includes("acessar") &&
    !window.location.toString().includes("nova-escola") &&
    !window.location.toString().includes("editar-escola")
  ) {
...
function getValorSelects() {
  ...
  $.ajax({
    ...
    $.each(data, function (index, item) {
      $("#situacaoFuncionamentoId").append(
        $("<option>", {
  ...
  $.ajax({
    ...
    $.each(data, function (index, item) {
      $("#categoriaEscolaPrivadaId").append(
        $("<option>", {
