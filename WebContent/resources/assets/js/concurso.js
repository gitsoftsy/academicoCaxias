...
$(document).ready(function () {
  ...
  $.ajax({
    ...
    preencherOpcoes(
      periodosLetivos,
      "#periodoLetivoOptionEdit",
      "#periodoLetivoIdEdit",
      "#periodoLetivoSearchEdit"
    );
  });
...
function preencherOpcoes(periodosLetivos, optionsListId, selectId, searchId) {
  ...
  $.each(periodosLetivos, function (index, item) {
    if (item.ativo === "S") {
      ...
      const optionHTML = `<li data-value="${item.idPeriodoLetivo}">${
        item.ano
      }/${item.periodo} - ${formatarPeriodo(item.tipoPeriodicidade)} - ${
        item.descricao
      }</li>`;
...
function alteraStatus(element) {
  ...
  $.ajax({
    url:
      url_base + `/concursos/${id}${status === "S" ? "/desativar" : "/ativar"}`,
    type: "put",
    error: function (e) {
      Swal.close();
...
function showModal(ref) {
  ...
  const periodoLetivoSelecionado = $(
    "#periodoLetivoIdEdit option:selected"
  ).text();
    ...
    error: function (e) {
      ...
      Swal.fire({
        ...
        text: e.responseJSON.message
          ...
          : "Não foi possível realizar esse comando!",
      });
    },
  }).done(function (data) {
    $("#cadastro-nome").val("");
