...
function getDados() {
	$.ajax({
		...
		.done(function(data) {
			...
			$("#periodoLetivo").val(`${data.periodoLetivo.periodo} - ${data.periodoLetivo.descricao}`)
	...
	var html = dados
		.map(function(item) {
			...
			return (
				...
				item.tipoIngresso +
				"</td>" +
				'<td class="d-flex justify-content-center">' + '<span style="width:50%; margin-right: 5px; height: 31px; padding: 8px; display: flex; align-items: center; justify-content: center;" class="btn btn-primary btn-sm" ' +
				'data-id=' + item.idAluno +
				' onclick="showModal(this)"><i class="fa-solid fa-file-lines "></i></span>' + ' </td>' +
				"</tr>"
			);
		})
		.join("");
